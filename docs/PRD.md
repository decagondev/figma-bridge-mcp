...
## 1. Objective
To bridge the gap between Figma designs and MCP-compatible AI coding agents (Cursor and Claude Code) for users on the Figma Personal/Starter tier, enabling "agentic" UI development where the LLM can autonomously pull resolved design tokens and layout structures.
...
- **Manual Friction**: Developers have to manually export files or copy-paste CSS constantly, breaking the "flow" of agentic coding in Cursor or Claude Code.

## 3. Scope and Assumptions
- Target Users: Developers using Figma Personal/Starter tier and an MCP-compatible AI coding agent (Cursor or Claude Code) for code generation.
- Supported Clients: Cursor (via `.cursor/mcp.json` stdio config) and Claude Code (via `claude mcp add` CLI or `.mcp.json` project config).
...
- Success Metrics:
  - Zero Hardcoding: No raw hex codes generated by the agent; all values must come from the synced tokens.
  - Reduced Latency: Syncing a frame to a code-ready context should take < 3 seconds.
  - Adoption: Measured by successful tool calls in Cursor and Claude Code without errors.
...
**User Stories**:
- **US2.1**: As an agent (Cursor or Claude Code), I want to sync design context so that I have up-to-date tokens.
  - **Features**:
...
**User Stories**:
- **US4.1**: As an MCP client (Cursor or Claude Code), I want to call tools via MCP so that I can integrate into workflows.
  - **Features**:
    - F4.1.1: Set up MCP server with SDK (stdio transport for both Cursor and Claude Code).
    - F4.1.2: Register sync_design_context and get_frame_structure tools.
    - F4.1.3: Handle errors and latency optimization.
    - F4.1.4: Document client-specific setup for Cursor (`.cursor/mcp.json`) and Claude Code (`claude mcp add` / `.mcp.json`).
...
- **US5.2**: As a user, I want easy iteration on design changes.
  ...
  - **Commits and Sub-Tasks**:
    ...
    - Commit: `test: Workflow simulation`
      - Sub-task: End-to-end test simulating Cursor and Claude Code tool calls.
...
- **Deployment**: Local run via `npm start`; package as CLI if needed. Provide setup docs for both Cursor and Claude Code.
